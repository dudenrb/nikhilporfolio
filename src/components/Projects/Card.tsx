...
import { Link } from "react-router-dom"; // Assuming you are using react-router-dom for Link
...
interface CardProps {
  ...
  liveLink?: string;
  githubLink: string;
  // Made 'icons' prop optional by adding '?'
  // Now, you don't have to provide it if it's not needed.
...
export function Card({
  ...
  liveLink,
  githubLink,
  icons, // 'icons' can now be undefined
}: CardProps) {
  return (
    <CardContainer className="max-w-full overflow-x-auto">
      ...
      >
        ...
        {/* Conditionally render icons only if 'icons' and 'icons.iconLink' exist */}
        {icons?.iconLink && icons.iconLink.length > 0 && (
